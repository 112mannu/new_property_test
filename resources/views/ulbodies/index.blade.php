  ...
  <main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-200">
      <div class="container mx-auto px-6 py-2">
          <div class="text-right">
            @can('UlbType create')
              <a href="{{route('admin.ulbodies.create')}}" class="bg-blue-500 text-white font-bold px-5 py-1 rounded focus:outline-none shadow hover:bg-blue-500 transition-colors ">New UL Bodies</a>
            @endcan
          </div>
          ...
          <table class="text-left w-full border-collapse">
            ...
            <tbody>
              @can('UlbType access')
                ...
                <td class="py-4 px-6 border-b border-grey-light">{{$ulb->city}}</td>
                <td class="py-4 px-6 border-b border-grey-light">{{$ulb->remark}}</td>
                  <td class="py-4 px-6 border-b border-grey-light text-right">

                    @can('UlbType edit')
                    <a href="{{route('admin.ulbodies.edit',$ulb->id)}}" class="text-grey-lighter font-bold py-1 px-3 rounded text-xs bg-green hover:bg-green-dark text-blue-400">Edit</a>
                    @endcan
